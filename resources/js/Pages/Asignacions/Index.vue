...
const { getAsignacions, setAsignacion, limpiarAsignacion, deleteAsignacion } =
    useAsignacions();
...
const accionesRow = () => {
    // detalle
    $("#table-asignacion").on("click", "button.detalle", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        axios.get(route("asignacions.show", id)).then((response) => {
            setAsignacion(response.data, true);
            accion_dialog_det.value = 1;
            open_dialog_det.value = true;
        });
    });
    // editar
    $("#table-asignacion").on("click", "button.editar", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        axios.get(route("asignacions.show", id)).then((response) => {
    ...
    // eliminar
    $("#table-asignacion").on("click", "button.eliminar", function (e) {
        e.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let respuesta = await deleteAsignacion(id);
...
<template>
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Asignacions</li>
    </ol>
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">Asignacions</h1>
    <!-- END page-header -->
